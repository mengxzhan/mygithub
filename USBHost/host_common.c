...
extern volatile boolean hid_test;
...
{ /* Body */
   ...
   uint_32                    cfg_end;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_attach_detach\n"));
   #endif
   ...
   USB_unlock();
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_attach_detach SUCCESSFUL\n"));
   #endif
} /* EndBody */
...
{ /* Body */
   ...
   DEV_INSTANCE_PTR           test_ptr = (DEV_INSTANCE_PTR)usb_host_ptr->DEVICE_LIST_PTR;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_busy_check\n"));
   #endif
   ...
   if (test_ptr == NULL)
   {
         ...
         DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_busy_check device not found\n"));
      #endif
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_busy_check SUCCESSFUL\n"));
   #endif
...
{ /* Body */
   ...
   DEV_INSTANCE_PTR           test_ptr = (DEV_INSTANCE_PTR)usb_host_ptr->DEVICE_LIST_PTR;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_validate\n"));
   #endif
   ...
   while ((test_ptr != dev_ptr) && (test_ptr != NULL))
      ...
      {  
         ...
         DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_validate SUCCESSFUL\n"));
      }
   #endif
...
{ /* Body */
   ...
   USB_STATUS        error;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_get_buffer\n"));
   #endif
   ...
   if (error != USB_OK) {
      USB_unlock();
         ...
         DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_get_buffer invalid device handle\n"));
      #endif
      return error;
   } /* Endif */
   ...
   USB_unlock();
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_get_buffer SUCCESSFUL\n"));
   #endif
   return error;
...
{ /* Body */
   ...
   USB_STATUS        error;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_get_class\n"));
   #endif
   ...
   if (error != USB_OK) {
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_get_class invalid device handle\n"));
   #endif
      return error;
   } /* Endif */
   ...
   USB_unlock();
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_get_class SUCCESSFUL\n"));
   #endif
   return error;
...
{ /* Body */
   ...
   #ifdef _HOST_DEBUG_
      DEBUG_LOG_TRACE("usb_hostdev_keep_class");
      DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_keep_class\n"));
   #endif

   #ifdef _HOST_DEBUG_
      DEBUG_LOG_TRACE("usb_hostdev_keep_class SUCCESSFUL");
      DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_keep_class SUCCESSFUL\n"));
   #endif
...
{ /* Body */
   ...
   DESCRIPTOR_UNION  ptr1, ptr2;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_get_descriptor\n"));
   #endif
   ...
   *descriptor = NULL; /* Assume the worst */
   ...
   if (usb_hostdev_validate(dev_handle) != USB_OK) {
      USB_unlock();
      #ifdef _HOST_DEBUG_
         DEBUG_LOG_TRACE("_usb_hostdev_get_descriptor invalid device handle");
         DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_get_descriptor invalid device handle\n"));
      #endif
      return USBERR_DEVICE_NOT_FOUND;
   } /* Endif */
   ...
   if (dev_ptr->state < DEVSTATE_CFG_READ) {
      USB_unlock();
         ...
         DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_get_descriptor device busy\n"));
      #endif
   ...
   if (desc_type == DESCTYPE_DEVICE) {
      *descriptor = &dev_ptr->dev_descriptor;
      USB_unlock();
         ...
         DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_get_descriptor SUCCESSFUL\n"));
      #endif
      return USB_OK;
   } /* EndIf */
   ...
   if (desc_type != DESCTYPE_CONFIG) {
      ...
      switch (desc_type) {
         case DESCTYPE_INTERFACE:
            if (ptr1.cfig->bNumInterfaces < desc_index) {
               USB_unlock();
                  ...
                  DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_get_descriptor no INTF descriptor\n"));
               #endif
               return USBERR_NO_DESCRIPTOR;
            } /* Endif */
            ...
            ** the type & length for interface descriptors */
            ...
            if (ptr1.intf == NULL) {
               USB_unlock();
                  ...
                  DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_get_descriptor no EP descriptor\n"));
               #endif
               return USBERR_NO_DESCRIPTOR;
            } /* Endif */
               ...
               (ptr1.intf->bLength != 9) )
            {
               ...
               USB_unlock();
                  ...
                  DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_get_descriptor no descriptor\n"));
               #endif
               return USBERR_NO_DESCRIPTOR;
            } /* EndIf */
            ...
            */
            if ((desc_index == 0) ||
               ...
               USB_unlock();
                  ...
                  DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_get_descriptor no EP0 descriptor\n"));
               #endif
               return USBERR_NO_DESCRIPTOR;
            } /* Endif */
         ...
         default: 
            USB_unlock(); 
            #ifdef _HOST_DEBUG_
               DEBUG_LOG_TRACE("_usb_hostdev_get_descriptor no STRING descriptor");
               DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_get_descriptor no STRING descriptor\n"));
            #endif
            return USBERR_NO_DESCRIPTOR;
      } /* EndSwitch */
   } /* EndIf */
...
{ /* Body */
   ...
   uint_8                     index;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_find_pipe_handle\n"));
   #endif
//   hid_test = TRUE;
   USB_lock();
   ...
   if ((desc.intf->bDescriptorType != DESCTYPE_INTERFACE)
      ...
      USB_unlock();
         ...
         DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_find_pipe_handle invalid interface\n"));
      #endif
      return NULL;
   } /* Endif */
            ...
            if (dev_mem->blktype == USB_MEMTYPE_PIPES) {
                  ...
                  && (pbs_ptr->intf_handle == desc.pntr))
               {
                  ...
                  USB_unlock();
                     ...
                     DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_find_pipe_handle SUCCESSFUL \n"));
                  #endif
                  return pipe_ptr;
               } /* Endif */
            } else {
   ...
   USB_unlock();
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_find_pipe_handle error \n"));
   #endif
   return NULL;
...
{ /* Body */
   ...
   uint_8                     m, n;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_get_pipe_handle\n"));
   #endif
   ...
   if (USB_OK != usb_hostdev_validate(pbs_ptr->dev_handle))
   {
         ...
         DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_get_pipe_handle invalid device handle\n"));
      #endif
      return NULL;
   ...
   switch (pipe_type) {
      case USB_CONTROL_PIPE:
            ...
            DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_get_pipe_handle SUCCESSFUL\n"));
         #endif
      ...
      default:
            ...
            DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_get_pipe_handle FAILED\n"));
         #endif
         return NULL;
   } /* EndSwitch */
   ...
   switch (pipe_direction) {
      ...
      default:
            ...
            DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_get_pipe_handle invalid direction\n"));
         #endif
         return NULL;
   } /* EndSwitch */
      ...
      (desc.intf->bLength != 9))
   {
         ...
         DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_get_pipe_handle invalid interface\n"));
      #endif
      return NULL;
   } /* Endif */
...
{ /* Body */
   ...
   USB_STATUS                    error;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_host_class_intf_init\n"));
   #endif
   ...
   if (error != USB_OK) {
      USB_unlock();
         ...
         DEBUGMSG(USB_DBG_LEVEL, ("usb_host_class_intf_init invalid device\n"));
      #endif
      return error;
   } /* EndIf */
         ...
         (intf_desc->bLength != sizeof(INTERFACE_DESCRIPTOR)))
   {
      USB_unlock();
         ...
         DEBUGMSG(USB_DBG_LEVEL, ("usb_host_class_intf_init interface not found\n"));
      #endif
   ...
   for (mem_ptr = dev_ptr->memlist; ;mem_ptr = mem_ptr->next)  {
      ...
      if (mem_ptr == NULL) {
         USB_unlock();
            ...
            DEBUGMSG(USB_DBG_LEVEL, ("usb_host_class_intf_init no interface memory\n"));
         #endif
         return USBERR_NO_INTERFACE;
      } /* EndIf */
   ...
   if (anchor_ptr == NULL) {
      USB_unlock ();
         ...
         DEBUGMSG(USB_DBG_LEVEL, ("usb_host_class_intf_init invalid parameter\n"));
      #endif
      return USBERR_INVALID_ANCHOR;
   } /* Endif */

   USB_memzero(class_intf_handle, mem_ptr->blksize);
   ...
   USB_unlock();
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_host_class_intf_init SUCCESSFUL\n"));
   #endif
   return USB_OK;
} /* EndBody */
...
{ /* Body */
   ...
   GENERAL_CLASS_PTR       gen_ptr;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_host_class_intf_validate\n"));
   #endif
   ...
   if (cc_ptr->code_key != 0 ) {
         ...
         DEBUGMSG(USB_DBG_LEVEL, ("usb_host_class_intf_validate SUCCESSFUL\n"));
      #endif
...
{ /* Body */
   uint_16  info_Vendor, info_Product, dev_Vendor, dev_Product;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_host_driver_info_match\n"));
   #endif
   ...
   if ((info_Vendor | info_Product) != 0) {
         ...
         (info_Product == dev_Product))
      {
            ...
            DEBUGMSG(USB_DBG_LEVEL, ("usb_host_driver_info_match PID, VID match\n"));
         #endif
         return TRUE;
      } /* Endif */
      ...
      if (dev_ptr->dev_descriptor.bDeviceClass == 0xFF) {
         ...
         {
            #ifdef _HOST_DEBUG_
               DEBUG_LOG_TRACE("usb_host_driver_info_match VID, subclass match");
               DEBUGMSG(USB_DBG_LEVEL, ("usb_host_driver_info_match VID, subclass match\n"));
            #endif
            return TRUE;
         } /* Endif */
         ...
         {
            #ifdef _HOST_DEBUG_
               DEBUG_LOG_TRACE("usb_host_driver_info_match class, subclass match");
               DEBUGMSG(USB_DBG_LEVEL, ("usb_host_driver_info_match class, subclass match\n"));
            #endif
            return TRUE;
         }
      } /* Endif */
      ...
      if (intf_ptr->bInterfaceClass == 0xFF) {
         ...
         {
            #ifdef _HOST_DEBUG_
               DEBUG_LOG_TRACE("usb_host_driver_info_match interface vid,class,subclass match");
               DEBUGMSG(USB_DBG_LEVEL, ("usb_host_driver_info_match interface vid,class,subclass match\n"));
            #endif
            return TRUE;
         } /* Endif */
   ...
   if (intf_ptr->bInterfaceClass != 0xFF) {
      ...
      {
         #ifdef _HOST_DEBUG_
            DEBUG_LOG_TRACE("usb_host_driver_info_match interface class,subclass match");
            DEBUGMSG(USB_DBG_LEVEL, ("usb_host_driver_info_match interface class,subclass match\n"));
         #endif
         return TRUE;
      } /* Endif */
   } /* Endif */
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_host_driver_info_match interface No Match\n"));
   #endif
...
{ /* Body */
   ...
   _usb_interface_descriptor_handle intf_handle;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_host_driver_info_scan\n"));
   #endif
   ...
   if (dev_ptr->host == NULL)
   {
         ...
         DEBUGMSG(USB_DBG_LEVEL, ("usb_host_driver_info_scan NULL host pointer\n"));
      #endif
      return;
   ...
   if (info_ptr) {
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_host_driver_info_scan SUCCESSFUL\n"));
   #endif
...
{ /* Body */
         ...
         DEBUGMSG(USB_DBG_LEVEL, ("usb_host_driver_info_nonzero\n"));
      #endif
   ...
   if (info_ptr->idProduct[1] != 0)
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_host_driver_info_nonzero FAILURE\n"));
   #endif
...
{ /* Body */
   ...
   uint_32                    intf_class, map_class;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_host_map_class_intf\n"));
   #endif
   ...
   for (map_ptr = &class_interface_map[0]; ; map_ptr++) {
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_host_map_class_intf SUCCESSFUL\n"));
   #endif
...
{ /* Body */
   ...
   USB_HOST_STATE_STRUCT_PTR host_struct_ptr;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("_usb_host_driver_info_register\n"));
   #endif
   ...
   if (host_handle == NULL || info_table_ptr == NULL) {
      USB_unlock();
         ...
         DEBUGMSG(USB_DBG_LEVEL, ("_usb_host_driver_info_register, NULL pointers\n"));
      #endif
   ...
   USB_unlock();
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("_usb_host_driver_info_register SUCCESSFUL\n"));
   #endif
   return USB_OK;
} /* EndBody */
...
{ /* Body */
   ...
   USB_lock();
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_select_config\n"));
   #endif
   ...
   if (error != USB_OK) {
      USB_unlock();
         ...
         DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_select_config invalid device\n"));
      #endif
      return error;
   } /* Endif */
   ...
   USB_unlock();
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_select_config SUCCESSFUL\n"));
   #endif
   return USB_OK;
...
{ /* Body */
   ...
   uint_8                   intf_no;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_delete_interface\n"));
   #endif
   ...
   for (intf_no = 0; intf_no < dev->num_of_interfaces; intf_no++) {
      ...
      {
         #ifdef _HOST_DEBUG_
            DEBUG_LOG_TRACE("usb_hostdev_delete_interface audio interface");
            DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_delete_interface audio interface\n"));
         #endif
         return;
      }
...
{ /* Body */
   ...
   uint_32  result;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_mask_class_etc\n"));
   #endif
...
{ /* Body */
   ...
   USB_STATUS               error;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_open_pipe\n"));
   #endif
   ...
   if (error != USB_OK)
   {
         ...
         DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_open_pipe failed\n"));
      #endif
      return error;
   }
   DEBUGMSG(1, ("usb_hostdev_open_pipe after get descriptor %d \n", (desc.ndpt->bEndpointAddress) & ENDPOINT_MASK));
   dev = (DEV_INSTANCE_PTR)dev_handle;
   ...
   error = _usb_host_open_pipe(dev->host,
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_open_pipe SUCCESSFUL\n"));
   #endif
...
{ /* Body */
   ...
   uint_8                        ep_num, intf_no;
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_select_interface\n"));
   #endif
   ...
   DEBUGMSG(USB_DBG_LEVEL, ("before nonzero\n"));
   if (usb_host_driver_info_nonzero(info_ptr)) {
	   DEBUGMSG(USB_DBG_LEVEL, ("nonzero\n"));
      ...
      if (class_ptr == NULL){
    	 DEBUGMSG(USB_DBG_LEVEL, ("null class_ptr\n"));
         goto BadExit;
      }
      ...
      if (class_map == NULL){
    	 DEBUGMSG(USB_DBG_LEVEL, ("null class_map\n"));
         goto BadExit;
      }
      ...
      if (!usb_hostdev_keep_class(intf)) {
         ...
         if (error != USB_OK){
        	DEBUGMSG(USB_DBG_LEVEL, ("get_class_error\n"));
            goto BadExit;
         }
      } /* EndIf */
   ...
   USB_unlock();
   #ifdef _HOST_DEBUG_
      DEBUG_LOG_TRACE("_usb_hostdev_select_interface SUCESSFUL");
      DEBUGMSG(USB_DBG_LEVEL, ("_usb_hostdev_select_interface SUCESSFUL\n"));
   #endif
   return USB_OK;
...
{ /* Body */
      ...
      DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_tr_init\n"));
   #endif
//   DEBUGMSG(USB_DBG_LEVEL, ("tr_init\n"));
   USB_lock();
   ...
   *tr_ptr = tr_prototype;
   ...
   USB_unlock();
//   if(hid_test){
//	   DEBUGMSG(USB_DBG_LEVEL, ("hid_test\n"));
//   }
   #ifdef _HOST_DEBUG_
...
{ /* Body */
   #ifdef _HOST_DEBUG_
      DEBUG_LOG_TRACE("usb_hostdev_tr_init SUCCESSFUL");
      DEBUGMSG(USB_DBG_LEVEL, ("usb_hostdev_tr_init SUCCESSFUL\n"));
   #endif
...
/*FUNCTION*----------------------------------------------------------------
*
* Function Name  : ch9_get_status_ex (enhance version of get_status()
* Returned Value : None
* Comments       :
*     USB Chapter 9 GET_STATUS
*END*--------------------------------------------------------------------*/
USB_STATUS ch9_get_status_ex
   (
      _usb_device_instance_handle   dev_handle,
	  uint_16_ptr                	pBuf,
	  tr_callback 					callback
   )
{
   ...
   /* Get Status on USB device */
   request.BMREQUESTTYPE = REQ_TYPE_IN;
   ...
   status = _usb_hostdev_cntrl_request (dev_handle,
      &request, (uchar_ptr)pBuf, callback, NULL);

   if (status == USB_OK || status == USB_STATUS_TRANSFER_QUEUED)
   {
      DEBUGMSG(USB_DBG_LEVEL,("%s: SUCCESSFUL", __FUNCTION__));
   }
   else
   {
...
/****************************************************************************
   Common version of enable period schedule
   if periodic schedule is not already enabled, enable it.
****************************************************************************/
/*FUNCTION*----------------------------------------------------------------
*
* Function Name  : usb_enable_period_schedule
* Returned Value : Error status
* Comments       :
*     Common version of enable period schedule - If periodic schedule
*     is not already enabled, enable it.
*END*--------------------------------------------------------------------*/
USB_STATUS usb_enable_period_schedule
(
