...
*****************************************************************/
   ...
   if (pipe_descr_ptr->PIPE_ID > USB_MAX_PIPES) {
      #ifdef _HOST_DEBUG_
         DEBUG_LOG_TRACE("_usb_host_recv_data inalid pipe handle");
      #endif
   ...
   if (pipe_descr_ptr->STATUS != USB_STATUS_IDLE) {
      USB_unlock();
      #ifdef _HOST_DEBUG_
         DEBUG_LOG_TRACE("_usb_host_recv_data transfer in progress");
      #endif
      ...
      */
      ...
      USB_dcache_invalidate_mlines((void *)pipe_descr_ptr->RX_PTR,
...
*****************************************************************/
   ...
   /*Must Flush and Invalidate the buffer before sending
   /receiving the data in it */
   USB_dcache_invalidate_mlines((void *)pipe_tr_ptr->RX_BUFFER,
