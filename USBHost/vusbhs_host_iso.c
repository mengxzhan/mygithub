...
{ /* Body */
   ...
   We ensure that all fields inside it are NULL beforr we try to link it.
   ******************************************************************************/
   next = 0;
   USB_memzero(struct_to_link_list,MAX_ITD_DESCRS*sizeof(uint_32));    
   ...
   *******************************************************************************/
   ...
   for(i=0;i<no_of_itds;i++)
   {
         ...
         **********************************************************************/
         USB_memzero((uchar_ptr)ITD_ptr,16*sizeof(uint_32));
         ...
         ITD_ptr->SCRATCH_PTR->PRIVATE = handle;
   
         /* Set the Terminate bit */
         ITD_ptr->NEXT_LINK_PTR = EHCI_ITD_T_BIT;
         
         /*store endpoint number and device address */
         ITD_ptr->BUFFER_PAGE_PTR_LIST[0] =  (pipe_descr_ptr->ENDPOINT_NUMBER << EHCI_ITD_EP_BIT_POS) |
                                              pipe_descr_ptr->DEVICE_ADDRESS;

         /*store max packet size and direction */
         ITD_ptr->BUFFER_PAGE_PTR_LIST[1] =  pipe_descr_ptr->MAX_PACKET_SIZE | 
            ...
            if(pipe_descr_ptr->BWIDTH_SLOTS[j])
            {
                  ...
                  **************************************************************/
                  ...
                  else
                  {
                     ...
                     length_scheduled =  pipe_descr_ptr->TRS_PER_UFRAME*pipe_descr_ptr->MAX_PACKET_SIZE;
...
{ /* Body */
   ...
   uint_32                                         no_of_sitds=0/*, clean warning no_of_frames=0*/;
   uint_32                                         i,next;
   // clean warning uint_8                                          max_slots_per_frame=0;
   ...
   ******************************************************************************/
   next = 0;
   USB_memzero(struct_to_link_list,MAX_SITD_DESCRS*sizeof(uint_32));    
   ...
   *******************************************************************************/
   ...
   for(i=0;i<no_of_sitds;i++)
   {
         ...
         **********************************************************************/
         ...
         defined by EHCI specs). Fill the necessary fields.
         **********************************************************************/
         USB_memzero((pointer)SITD_ptr, 7 * sizeof(uint_32));
         ...
         SITD_ptr->SCRATCH_PTR->PRIVATE = handle;
         ...
         SITD_ptr->EP_CAPAB_CHARAC =  (sitd_direction_bit              << EHCI_SITD_DIRECTION_BIT_POS) |
         ...
         SITD_ptr->UFRAME_SCHED_CTL = (pipe_descr_ptr->COMPLETE_SPLIT << EHCI_SITD_COMPLETE_SPLIT_MASK_BIT_POS) |
         ...
         **************************************************************/
         ...
         tp_bits = (length_scheduled < EHCI_START_SPLIT_MAX_BUDGET) ? EHCI_SITD_TP_ALL : EHCI_SITD_TP_BEGIN;
...
)
{
   ...
   EHCI_ITD_STRUCT_PTR                          ITD_ptr;
   EHCI_SITD_STRUCT_PTR                         SITD_ptr;
   uint_32                                      i,j,frame,uframe,start,interval;
